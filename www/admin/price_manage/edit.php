...
<script>
    ...
    function initTinyMCEForText(selector) {
        const priceManageConfig = addImageUploadConfig(TinyMCEConfig.full, '/admin/price_manage/api/upload_image.php', '/');
    ...
    function openNewlyCreatedCard() {
        ...
        if (newContentId) {
            ...
            // URLからパラメータを削除
            window.history.replaceState({}, document.title, window.location.pathname + '?id=' + setId);
        }
    }
