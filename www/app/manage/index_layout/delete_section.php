...
function extractImagePathsFromHtml($html) {
    ...
    if (preg_match_all('/src=["\']([^"\']+)["\']/', $html, $matches)) {
        foreach ($matches[1] as $src) {
            if (strpos($src, '/manage/index_layout/api/uploads/') !== false) {
                $paths[] = $src;
            }
        }
    }
...
// デフォルトセクションのsection_keyリスト（削除不可）
...
try {
    ...
    // セクション情報を取得
    $stmt = $pdo->prepare("SELECT section_key, section_type, config FROM index_layout_sections WHERE id = ? AND tenant_id = ?");
    ...
    // 関連するバナー画像を取得
    $stmt = $pdo->prepare("SELECT image_path FROM index_layout_banners WHERE section_id = ? AND tenant_id = ?");
    $stmt->execute([$id, $tenantId]);
    ...
    // 関連するバナーレコードを削除
    $stmt = $pdo->prepare("DELETE FROM index_layout_banners WHERE section_id = ? AND tenant_id = ?");
    $stmt->execute([$id, $tenantId]);
    $deletedBanners = $stmt->rowCount();
    
    // セクションを削除
    $stmt = $pdo->prepare("DELETE FROM index_layout_sections WHERE id = ? AND tenant_id = ?");
    $stmt->execute([$id, $tenantId]);
