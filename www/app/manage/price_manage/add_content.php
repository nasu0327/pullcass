...
if (!$setId || !in_array($contentType, ['price_table', 'price_table_1col', 'banner', 'text'])) {
...
try {
    ...
    // price_table_1col は内部的には price_table として扱う
    $dbContentType = ($contentType === 'price_table_1col') ? 'price_table' : $contentType;

    $stmt = $pdo->prepare("
        INSERT INTO price_contents (set_id, content_type, admin_title, display_order, is_active)
        VALUES (?, ?, ?, ?, 1)
    ");
    $stmt->execute([$setId, $dbContentType, $adminTitle, $maxOrder]);
    $contentId = $pdo->lastInsertId();

    // 詳細テーブルにも初期データを追加（表示名・列名も空でプレースホルダー表示）
    if ($contentType === 'price_table') {
        $stmt = $pdo->prepare("
            INSERT INTO price_tables (content_id, column_count, table_name, column1_header, column2_header, note)
            VALUES (?, 2, '', '', '', '')
        ");
        $stmt->execute([$contentId]);
    } elseif ($contentType === 'price_table_1col') {
        $stmt = $pdo->prepare("
            INSERT INTO price_tables (content_id, column_count, table_name, column1_header, column2_header, note)
            VALUES (?, 1, '', '', '', '')
        ");
