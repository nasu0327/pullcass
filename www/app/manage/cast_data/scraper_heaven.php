...
try {
    ...
    $stmt = $pdo->prepare("
        ...
        WHERE tenant_id = ? AND scraping_type = 'heaven'
    ");
    ...
    try {
        ...
        // 統合元のデータ数を確認
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM tenant_cast_data_heaven WHERE tenant_id = ? AND checked = 1");
        $stmt->execute([$tenantId]);
        ...
        if ($sourceCount === 0) {
            ...
            $updateSql = "
                ...
                INNER JOIN tenant_cast_data_heaven s ON c.name = s.name AND c.tenant_id = s.tenant_id
                SET 
            ...
            $insertSql = "
                ...
                FROM tenant_cast_data_heaven s
            ...
            // active_sourceも自動的に更新
            $stmt = $pdo->prepare("
                INSERT INTO tenant_scraping_config (tenant_id, config_key, config_value) 
                VALUES (?, 'active_source', 'heaven')
                ON DUPLICATE KEY UPDATE config_value = 'heaven'
            ");
            $stmt->execute([$tenantId]);
            logOutput("✅ active_sourceを'heaven'に自動更新");
            logOutput("========================================");
        }
    } catch (Exception $e) {
