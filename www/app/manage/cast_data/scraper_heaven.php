...
try {
    ...
    // キャスト詳細ページのURLを収集（ヘブンネット固有のパターン）
    $urls = [];
    $heavenIds = [];
    $linkNodes = $xpath->query("//a[contains(@href, 'girlid-')]");
    
    foreach ($linkNodes as $node) {
        $href = $node->getAttribute('href');
        if (preg_match('/girlid-(\d+)/', $href, $m)) {
            $heavenId = $m[1];
            if (!isset($heavenIds[$heavenId])) {
                $fullUrl = strpos($href, 'http') === 0 ? $href : $baseUrl . $href;
                $urls[] = ['url' => $fullUrl, 'heaven_id' => $heavenId];
                $heavenIds[$heavenId] = true;
            }
        }
    }
    ...
    $stmt = $pdo->prepare("UPDATE tenant_cast_data_heaven SET checked = 0 WHERE tenant_id = ?");
    $stmt->execute([$tenantId]);
    ...
    foreach ($urls as $i => $castInfo) {
        ...
        $heavenId = $castInfo['heaven_id'];
        $castName = null;
        
        try {
            logOutput(sprintf("[%d/%d] 取得中: girlid-%s", $i + 1, $totalCasts, $heavenId));
            ...
            // キャスト名（ヘブンネット固有）
            $nameNode = $xpath2->query("//h1[contains(@class, 'girl_name')]|//span[contains(@class, 'girl_name')]")->item(0);
            if (!$nameNode) {
                $nameNode = $xpath2->query("//div[contains(@class, 'girl_profile')]//h1|//div[contains(@class, 'name')]//span")->item(0);
            }
            ...
            // プロフィールテーブルから取得
            $profileNodes = $xpath2->query("//table[contains(@class, 'profile')]//tr | //dl[contains(@class, 'profile')]//dd");
            foreach ($profileNodes as $node) {
                $text = trim($node->textContent);
                
                if (preg_match('/(\d{2})歳/', $text, $m)) {
                    $age = (int)$m[1];
                }
                if (preg_match('/T?(\d{3})/', $text, $m)) {
                    $height = (int)$m[1];
                }
                if (preg_match('/([A-K])カップ/i', $text, $m)) {
                    $cup = strtoupper($m[1]);
                }
                if (preg_match('/B\s*:?\s*(\d+)[^\d]+W\s*:?\s*(\d+)[^\d]+H\s*:?\s*(\d+)/i', $text, $m)) {
            ...
            $prTitle = '';
            $prText = '';
            $prNode = $xpath2->query("//div[contains(@class, 'comment') or contains(@class, 'pr')]")->item(0);
            if ($prNode) {
            ...
            // 画像
            $images = [];
            $imgNodes = $xpath2->query("//img[contains(@src, 'girlimg') or contains(@class, 'girl_photo')]");
            foreach ($imgNodes as $imgNode) {
                $src = $imgNode->getAttribute('src') ?: $imgNode->getAttribute('data-src');
                if ($src && strpos($src, 'noimage') === false && strpos($src, 'spacer') === false) {
            ...
            $scheduleTable = $xpath2->query("//table[contains(@class, 'schedule')]//td | //div[contains(@class, 'schedule')]//span");
            $dayIndex = 1;
            foreach ($scheduleTable as $cell) {
                if ($dayIndex > 7) break;
            ...
            // 新人フラグ
            $new = 0;
            $newNode = $xpath2->query("//*[contains(@class, 'new') or contains(@class, 'newface')]")->item(0);
            if ($newNode) {
                $new = 1;
            }
